...
public class DebuggerTest extends BaseTest {
    ...
    public void setUp() {
        ...
        doReturn(MIME_TYPE).when(fileReference).getMediaType();
        ...
        debugger = new JavaDebugger(service, dtoFactory, localStorageProvider, messageBusProvider, eventBus, fqnResolverFactory, appContext, javaDebuggerFileHandler,
                                    debuggerManager);
        // setup messageBus
    ...
    public void testAttachDebugger() throws Exception {
        ...
        JavaDebuggerConnectionContext connectionContext = mock(JavaDebuggerConnectionContext.class);
        doReturn(host).when(connectionContext).getHost();
        doReturn(port).when(connectionContext).getPort();
        ...
        doReturn(promiseJavaDebuggerInfo).when(service).connect(connectionContext);
        ...
        Promise<Void> result = debugger.attachDebugger(connectionContext);
    ...
    public void testAttachDebuggerWithConnection() throws Exception {
        ...
        try {
            debugger.attachDebugger(connectionContext);
    ...
    public void testDeleteAllBreakpoints() throws Exception {
        ...
        verify(promiseVoid).then(operationVoidCaptor.capture());
        operationVoidCaptor.getValue().apply(null);
        verify(observer).onAllBreakpointDeleted();
