            ...
            github.setConnector(new HttpConnector() {
                ...
                public HttpURLConnection connect(URL url) throws IOException {
                    ...
                    if (sourceUrl.contains("pulls")) {
                        return HttpConnector.DEFAULT.connect(URI.create(url.toString() + "&head=" + head).toURL());
                    }
                    return HttpConnector.DEFAULT.connect(url);
                }
            });
    ...
    public void updateSSHKey() throws ApiException {
        ...
        if (sshPair != null) {
            if (sshPair.getPublicKey() == null) {
                ...
                sshPair = sshServiceClient.generatePair(DtoFactory.newDto(GenerateSshPairRequest.class).withService("git")
                                                                                            .withName(host));
            }
        } else {
            sshPair = sshServiceClient.generatePair(DtoFactory.newDto(GenerateSshPairRequest.class).withService("git")
