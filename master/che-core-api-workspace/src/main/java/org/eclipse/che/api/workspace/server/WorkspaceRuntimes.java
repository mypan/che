...
public final class WorkspaceRuntimes {
     ...
     *         when component {@link #isPostConstructInvoked is stopped} or any
    ...
    public RuntimeDescriptor start(WorkspaceImpl workspace, String envName, boolean recover) throws ServerException,
        ...
        try {
            ...
            final Queue<MachineConfigImpl> queue = new ArrayDeque<>(devFirst(activeEnv.getMachineConfigs()));
            startQueues.put(workspace.getId(), queue);
        } finally {
     ...
     * non-dev machines first. During the workspace stop the workspace
     * is accessible with {@link WorkspaceStatus#STOPPING stopping} status.
    ...
    public void stop(String workspaceId) throws NotFoundException, ServerException, ConflictException {
        ...
        stopMachines(workspaceId, runtime);
    }
    ...
    private void cleanupStartResources(String workspaceId) {
        rwLock.writeLock().lock();
        try {
    ...
    private <T extends MachineConfig> List<T> devFirst(List<T> machines) {
        T devMachine = null;
    ...
    private <T extends Machine> T rmDev(List<T> machines) {
        T devMachine = null;
        for (final Iterator<T> it = machines.iterator(); it.hasNext() && devMachine == null; ) {
            final T next = it.next();
            if (next.getConfig().isDev()) {
                devMachine = next;
                it.remove();
            }
        }
    ...
    private void removeRuntime(String wsId) {
        rwLock.writeLock().lock();
        try {
    ...
    private void ensurePostConstructIsNotExecuted() throws ServerException {
        if (isPostConstructInvoked) {
            throw new ServerException("Could not perform operation after post construct invoked");
        }
    }
